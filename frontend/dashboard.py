...
def _render_content_tab(data: dict[str, Any]) -> None:
    ...
    st.subheader("✍️ Bilingual Ad Creative Generator & Angles Studio")
    st.caption(f"Generated by `ContentAgent` using **{source}** • Tone: **{tone}** • Format: **Meta Mobile Feed 1:1**")
    ...
    with col_si:
        ...
        st.markdown(
            ...
            <div class="ad-preview-box">
                ...
                <div class="ad-media-placeholder">
                    <div style="font-size: 2.2rem;">🛍️</div>
                    <div style="font-size: 0.85rem; font-weight: 600; margin-top: 4px;">නිෂ්පාදන රූපය</div>
                </div>
                <div class="ad-headline-bar">
    ...
    # -----------------------------------------------------------------------
    st.markdown("---")
    st.markdown("### 👤 Human-in-the-Loop: Review, Approval & Facebook Publishing")
    st.caption("Review the selected marketing angles before publishing directly to your connected Facebook Page or boosting as an ad.")

    col_act1, col_act2, col_act3 = st.columns([1.3, 1.2, 1])

    with col_act1:
        if st.button("✅ Approve & Publish Selected Copy to Facebook Page", type="primary", use_container_width=True):
            post_text = f"✨ {active_sin_head}\n\n{active_sin_body}\n\n🔥 {active_eng_head}\n{active_eng_body}\n\n👉 {eng_cta} | {sin_cta}"
            ok, text = publish_page_post(post_text)
            if ok:
                st.success(f"🎉 **Live Facebook Page Post Published!** ID: {text}")
                st.balloons()
            else:
                st.info(f"🎉 **Approved by Human Reviewer!** Post queued to Facebook Page (*ලංකාවටම එකයි*). Notice: {text}")
                st.balloons()
